		...
		<view class="tp-box tp-box-sizing tp-pd-l-r-30 pt-30">
			...
			<view>
				...
				<view class="tp-title" :style="{ marginBottom: '36rpx' }">设备监控
				    <view class="tp-add" style="margin-left:10px" > <!-- 使用.stop修饰符 -->
				        <image src="../../static/icon/notify.svg" v-if="!activeNotify" @click="toNotify"></image>
				        <image src="../../static/icon/notify-red.svg" v-else @click="toNotify"></image>
				    </view>
		...
		addEqp() {
			...
			uni.scanCode({
				success: function (res) {
					...
					uni.navigateTo({
						url: '../sub/fishery-monitor/addMonitor?code=' + result + '&id=' + _this.currentGroup.id,
					})
				}
			});
		},
